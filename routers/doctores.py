...
def crear_perfil_doctor(doctor: schemas.DoctorCreate, db: Session = Depends(get_db)):
    """Crea el perfil profesional de un doctor"""

    # Verificar que el usuario existe
    usuario = (
        db.query(models.Usuario).filter(models.Usuario.id == doctor.usuario_id).first()
    )
    if not usuario:
        raise HTTPException(
    ...
    if usuario.tipo_usuario != models.TipoUsuarioEnum.DOCTOR:
        raise HTTPException(
    ...
    doctor_existente = (
        ...
        .filter(models.Doctor.usuario_id == doctor.usuario_id)
        .first()
    )
    ...
    cedula_existente = (
        db.query(models.Doctor)
        .filter(models.Doctor.cedula_profesional == doctor.cedula_profesional)
        .first()
    )
    ...
    nuevo_doctor = models.Doctor(**doctor.model_dump())
...
@router.get("", response_model=List[schemas.DoctorCompleto])
...
@router.get("/{doctor_id}", response_model=schemas.DoctorCompleto)
...
@router.get("/usuario/{usuario_id}", response_model=schemas.DoctorCompleto)
def obtener_doctor_por_usuario(usuario_id: int, db: Session = Depends(get_db)):
    """Obtiene el perfil de doctor asociado a un usuario"""

    doctor = (
        db.query(models.Doctor).filter(models.Doctor.usuario_id == usuario_id).first()
    )
...
@router.get("/especialidad/{especialidad}", response_model=List[schemas.DoctorCompleto])
def obtener_doctores_por_especialidad(
    especialidad: models.EspecialidadEnum, db: Session = Depends(get_db)
):
    """Obtiene todos los doctores de una especialidad específica"""

    doctores = (
        db.query(models.Doctor).filter(models.Doctor.especialidad == especialidad).all()
    )
...
def actualizar_doctor(
    doctor_id: int, doctor_data: schemas.DoctorBase, db: Session = Depends(get_db)
):
    ...
    if (
        ...
        cedula_existente = (
            db.query(models.Doctor)
            .filter(models.Doctor.cedula_profesional == doctor_data.cedula_profesional)
            .first()
        )
